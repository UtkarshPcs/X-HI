                    ...
                    <div style={{ padding: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      {(q.explanation || q.concept || q.misconception) && (
                        <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '8px', borderLeft: '3px solid #3b82f6', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                          {q.concept && <div><span style={{ color: '#38bdf8', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '4px', display: 'block' }}>Concept: </span><div className="custom-md" style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.9rem' }}><ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]}>{formatMath(q.concept)}</ReactMarkdown></div></div>}
                          {q.explanation && <div><span style={{ color: '#38bdf8', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '4px', display: 'block' }}>Explanation: </span><div className="custom-md" style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.9rem' }}><ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]}>{formatMath(q.explanation)}</ReactMarkdown></div></div>}
                          {q.misconception && <div><span style={{ color: '#f43f5e', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '4px', display: 'block' }}>Common Misconception: </span><div className="custom-md" style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.9rem' }}><ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]}>{formatMath(q.misconception)}</ReactMarkdown></div></div>}
                        </div>
                      )}
                    </div>
                  )}
