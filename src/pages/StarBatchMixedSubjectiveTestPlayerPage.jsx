...
import { getAllSubjectiveTests, submitSubjectiveTestAttempt, getUserSubjectiveTestAttemptsForTest } from '../services/starBatchSubjectiveTestService';
...
export default function StarBatchMixedSubjectiveTestPlayerPage() {
  const { currentUser } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const isMixed = true; // Always true for this page
  ...
  const targetMarks = parseInt(searchParams.get('marks')) || 40;
  ...
  async function fetchTest() {
    setLoading(true);
    try {
      const allTests = await getAllSubjectiveTests();
      const mixedTests = allTests.filter(t => testIds.includes(t.id));
      
      if (mixedTests.length === 0) throw new Error("No tests found for the selected chapters.");

      const allCombinedQuestions = [];
      mixedTests.forEach(t => {
        if (t.questions) {
           t.questions.forEach(q => {
             allCombinedQuestions.push({ ...q, originalTestId: t.id, originalChapterId: t.chapterId });
           });
        }
      });
      ...
      const [history, userBookmarks] = await Promise.all([
        getUserSubjectiveTestAttemptsForTest(userId, `mixed_${subject}`),
        getUserBookmarks(userId)
      ]);
      
      const bSet = new Set(userBookmarks.filter(b => testIds.includes(b.testId)).map(b => b.questionIndex));
      setBookmarks(bSet);
      ...
      allCombinedQuestions.forEach((q, idx) => {
      ...
      const multiplier = targetMarks === 80 ? 4 : (targetMarks === 40 ? 2 : 1);
      let reqCounts;
      ...
      setTest({
         id: `mixed_${subject}`,
         chapterId: 'mixed',
         title: `Mixed Subjective Test - ${subject}`
      });
  ...
  const handleToggleBookmark = async () => {
    ...
    try {
      await toggleBookmark(currentUser.id || currentUser.phone, {
        testId: q.originalTestId || test.id,
        testTitle: test.title,
        chapterId: q.originalChapterId || test.chapterId,
  ...
  const handleReport = async () => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        setReporting(true);
        try {
          await reportTestQuestion(q.originalTestId || test.id, q.originalIndex, q.originalChapterId || test.chapterId);
